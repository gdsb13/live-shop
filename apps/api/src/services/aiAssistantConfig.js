...
function buildSystemPrompt(sessionContext) {
  const lines = [
    ...
    '- checkout: ONLY after explicit affirmative confirmation to place the order (see CHECKOUT FLOW). Never say the order is confirmed without a successful checkout tool result that includes orderId.',
    ...
    '8. ONLY then call checkout with the chosen paymentMethod (and deliveryPin when used).',
